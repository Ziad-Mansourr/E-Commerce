...
export default function BrandDetails() {
  ...
  const wishListIds = useMemo(() => {
    return new Set(wish?.data?.map((item) => item.id));
  }, [wish]);
  ...
  const filteredProducts = useMemo(() => {
    return relatedProduct.filter((product) => product.brand.slug === brands);
  ...
  return (
    <>
      <div className="grid grid-cols-12 gap-5 w-[90%] m-auto">
        {loading? <Loading/> :(relatedProduct != "") ? filteredProducts?.map((product) =>
          <div key={product.id} className="relative group col-span-6 md:col-span-4 lg:col-span-3 xl:col-span-2  overflow-hidden  hover:scale-[1.059] duration-300 rounded-lg shadow-lg p-3 my-2 border-gray-400">
            ...
            {wishListIds.has(product.id) ?
              <button onClick={() => deleteWish(product.id)} className='absolute group-hover:right-6  transition-all duration-[.4s] top-6 -right-10 p-0 bg-transparent'><i className="text-2xl  text-green-400 fa-solid fa-heart" /> </button>
              :
              <button onClick={() => addWish(product.id)} className='z-10 absolute group-hover:right-6  transition-all duration-[.4s] top-6 -right-10 p-0 bg-transparent'><i className="fa-regular fa-heart text-2xl  text-green-400" />  </button>
            }
